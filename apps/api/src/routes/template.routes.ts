...
function extractTemplateCode(url: string) {
...
router.post('/import', async (req, res) => {
  try {
    ...
    let existingTemplate = await AITemplate.findOne({ code });
    if (existingTemplate) {
      // Just update it
      ...
      existingTemplate = await AITemplate.create({
        code,
...
router.get('/', async (req, res) => {
  try {
    const templates = await AITemplate.find().sort({ createdAt: -1 });
    res.json(templates);
  } catch (error) {
