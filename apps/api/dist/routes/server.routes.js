...
router.get('/:id/settings', async (req, res) => {
    try {
        ...
        let server = await database_1.Server.findOne({ serverId });
        if (!server) {
            server = await database_1.Server.create({
                serverId,
...
router.post('/:id/settings', async (req, res) => {
    try {
        const serverId = req.params.id;
        const { raidMode, antiLink, antiSpamSensitivity, logChannelId, muteDuration, blacklistedWords } = req.body;
        if (!serverId) {
            return res.status(400).json({ error: 'serverId is required' });
        }
